...
public class DefaultChannelOutboundBuffer extends ChannelOutboundBuffer {
     ...
     */
    @Override
    public final int add(Object msg, ChannelPromise promise) {
        msg = beforeAdd(msg);
        int size = ((AbstractChannel) channel).estimatorHandle().size(msg);
        if (size < 0) {
            size = 0;
        }
    ...
    @Override
    public final int remove0(Throwable cause) {
        Entry e = buffer[flushed];
        Object msg = e.msg;
        ChannelPromise promise = e.promise;
        int size = e.pendingSize;
