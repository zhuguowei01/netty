...
public final class AsynchronousDnsResolver {
    ...
    private final class DnsResponseHandler extends SimpleChannelInboundHandler<DnsResponse> {
        ...
        public void messageReceived(ChannelHandlerContext ctx, DnsResponse response) throws Exception {
            ResolverDnsQuery<?> query = queries.remove(response.header().id());
            if (query != null) {
                if (response.header().responseCode() == DnsResponseCode.NOERROR) {
                    ...
                    query.setFailure(new DnsResponseException(response.header().responseCode()));
                }
            }
        }
        ...
        public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
            ...
            queries.put(query.header().id(), query);
            promise.addListener(new ChannelFutureListener() {
                @Override
                public void operationComplete(ChannelFuture future) throws Exception {
                    if (!future.isSuccess()) {
                        queries.remove(query.header().id());
